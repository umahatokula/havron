...
trait PaymentTrait{
    ...
    protected function getPaymentMethodsList($country = null){
		...
		// CREDIT CARD
		if($ps->checkIsAllowed($country, 'paystack'))
			$list[] = [
				'title' => trans('pixel.shop::lang.fields.credit_card'),
				'gateway' => 'cc',
				'partial' => $this->renderPartial('@paystack', ['settings' => $ps])
			];

		// BANK TRANSFER
		if($ps->checkIsAllowed($country, 'flutterwave'))
			$list[] = [
				'title' => trans('pixel.shop::lang.fields.bank_transfer'),
				'gateway' => 'bank_transfer',
				'partial' => $this->renderPartial('@flutterwave', ['settings' => $ps, 'cart' => $cart])
			];

		// CASH ON DELIVERY
		if($ps->checkIsAllowed($country, 'remita'))
			$list[] = [
				'title' => trans('pixel.shop::lang.fields.cash_on_delivery'),
				'gateway' => 'cash_on_delivery',
				'partial' => $this->renderPartial('@payu', ['settings' => $ps])
			];

		// PAYPAL IPN
		if($ps->checkIsAllowed($country, 'payu'))
			$list[] = [
				'title' => trans('pixel.shop::lang.fields.paypal'),
				'gateway' => 'paypal',
				'partial' => $this->renderPartial('@method_paypal', ['settings' => $ps])
			];
